...
function delete_import_fields($text=true)
{
	...
	if($text == true)
	{
		$output->update_progress_bar(0, "Usuwanie tabeli ".TABLE_PREFIX."trackers.");
	}
	$db->drop_table("trackers");

	if($text == true)
	{
		$output->update_progress_bar(0, "Usuwanie tabeli ".TABLE_PREFIX."post_trackers.");
	}
	$db->drop_table("post_trackers");

	if($text == true)
	{
		$output->update_progress_bar(0, "Usuwanie tabeli ".TABLE_PREFIX."privatemessage_trackers.");
	}
	...
	foreach($drop_list as $table => $columns)
	{
		...
		if($text == true)
		{
			$output->update_progress_bar($progress, "Usuwanie kolumn ".$columns_list." z tabeli ".TABLE_PREFIX.$table);
			$progress += $increment;
		}
...
function create_import_fields($text=true)
{
	...
	if($text == true)
	{
		...
		echo "<br />Tworzenie pól do śledzenia danych podczas procesu importu/łączenia (to może chwilę potrwać)...";
		flush();
	}
	...
	if($text == true)
	{
		$output->update_progress_bar(0, "Tworzenie tabeli ".TABLE_PREFIX."trackers.");
	}
	...
	if($text == true)
	{
		$output->update_progress_bar(0, "Tworzenie tabeli ".TABLE_PREFIX."post_trackers.");
	}
	...
	foreach($add_list['int'] as $table => $columns)
	{
		...
		if($text == true)
		{
			$output->update_progress_bar($progress, "Dodawanie kolumn typu integer ".$columns_list." do tabeli ".TABLE_PREFIX.$table);
			$progress += $increment;
		}
	...
	foreach($add_list['text'] as $table => $columns)
	{
		...
		if($text == true)
		{
			$output->update_progress_bar($progress, "Dodawanie kolumn typu tekstowego ".$columns_list." do tabeli ".TABLE_PREFIX.$table);
			$progress += $increment;
		}
	}
...
function my_friendly_time($timestamp)
{
	...
	if($months)
	{
		$string .= "{$comma}{$months} miesięcy";
		$comma = ", ";
	}

	if($days)
	{
		$string .= "{$comma}{$days} dni";
		$comma = ", ";
	}

	if($hours)
	{
		$string .= "{$comma}{$hours} godzin";
		$comma = ", ";
	}

	if($minutes)
	{
		$string .= "{$comma}{$minutes} minut";
		$comma = ", ";
	}

	if($seconds)
	{
		$string .= "{$comma}{$seconds} sekund";
		$comma = ", ";
	}
